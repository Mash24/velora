...
export function adminButtonClass(variant: ButtonVariant = "primary") {
  const base =
    "inline-flex max-w-full min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-center text-sm font-semibold transition disabled:opacity-60";
  switch (variant) {
    case "secondary":
      return `${base} border border-navy/15 bg-white text-navy shadow-sm hover:bg-sand/50`;
    case "ghost":
      return `${base} font-medium text-navy/70 hover:bg-white/80 hover:text-navy`;
    case "danger":
      return `${base} font-medium text-coral hover:bg-coral/5`;
    default:
      return `${base} bg-teal text-white shadow-sm hover:bg-teal/90`;
  }
}
...
export function AdminPageHeader({
  ...
  return (
    <div className="mb-6 min-w-0 sm:mb-8">
      {backHref ? (
        <Link href={backHref} className={`${adminButtonClass("secondary")} mb-3 w-full sm:w-auto`}>
          ← {backLabel}
        </Link>
      ) : null}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0 flex-1">
          <h1 className="break-anywhere text-[clamp(1.25rem,4.2vw,1.875rem)] font-semibold tracking-tight text-navy">
            {title}
          </h1>
          {description ? (
            <p className="mt-1.5 max-w-2xl text-sm leading-6 text-navy/70">{description}</p>
          ) : null}
          {meta ? <div className="mt-2 text-sm text-navy/65">{meta}</div> : null}
        </div>
        {actions ? (
          <div className="flex w-full min-w-0 shrink-0 flex-col gap-2 sm:w-auto sm:max-w-[min(100%,22rem)] sm:flex-row sm:flex-wrap sm:justify-end lg:max-w-none">
            {actions}
          </div>
        ) : null}
      </div>
...
export function AdminCard({
  ...
  return (
    <section
      className={`min-w-0 rounded-2xl border border-navy/8 bg-white shadow-[0_1px_2px_rgba(22,52,76,0.04),0_10px_28px_rgba(22,52,76,0.05)] ${padding} ${className}`}
    >
      {title ? (
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h2 className="break-anywhere font-semibold text-navy">{title}</h2>
            {description ? <p className="mt-1 text-sm text-navy/70">{description}</p> : null}
          </div>
          {action ? <div className="min-w-0 shrink-0 sm:max-w-[min(100%,16rem)]">{action}</div> : null}
        </div>
      ) : null}
...
export function AdminStatCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-sm font-semibold text-teal transition group-hover:underline">
        Go there →
      </p>
    </Link>
  );
...
export function AdminFilterRow({
  ...
  return <div className={`admin-filter-row ${className}`}>{children}</div>;
}
